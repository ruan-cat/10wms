...
- [x] 33.1 移除自动路由相关导入
  ...
  - 从 main/src/router/index.ts 移除 vite-plugin-vue-meta-layouts 相关导入
...
- [x] 44. 补全缺失页面迁移 - P2 中优先级页面（库存管理模块）

- [x] 44.1 迁移库存管理模块剩余页面（第一批）
...
- [x] 45. 补全缺失页面迁移 - P2 中优先级页面（出库管理模块）

- [x] 45.1 迁移出库管理模块剩余页面
  - 出库类型 (types) - 原项目中只是类型定义文件，非页面组件，无需迁移
  - _Requirements: 4.5_
...
- [x] 46. 补全缺失页面迁移 - P2 中优先级页面（采购管理模块）

- [x] 46.1 迁移采购管理模块剩余页面（第一批）
...
- [x] 48. Checkpoint - P1 和 P2 补全页面验证
  - P2 中优先级页面迁移已全部完成（36/36，100%）

- [x] 49. 补全缺失页面迁移 - P3 低优先级页面

- [x] 49.1 迁移消息中间件模块剩余页面
...
- [x] 49.2 更新消息中间件模块路由配置
  - 创建 message.ts 路由（添加所有页面）
  - 配置路由元信息
  - 测试路由访问
  - _Requirements: 15.1, 15.2, 15.3, 15.4_
