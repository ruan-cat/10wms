...
const tableColumns = ref<SimpleDataTableColumn<TableData>[]>([
	{ prop: "attributeCode", label: "产品属性编码", width: 200 },
	{ prop: "attributeName", label: "产品属性名称", width: 200 },
]);
		...
		.then(async () => {
			try {
				await http.requestCompat({
					url: "/api/product-attribute/delete",
					method: "post",
...
<template>
	<div class="product-attribute-page">
		...
		<el-dialog
			...
			:title="isViewMode ? '查看产品属性' : dialogType === 0 ? '新增产品属性' : '编辑产品属性'"
			width="600px"
			:before-close="handleDialogClose"
		>
			<ElForm :model="form" label-position="right" label-width="150px">
				<h2>表单信息管理</h2>
				<ElFormItem prop="attributeCode" label="产品属性编码：">
					<ElInput v-model="form.attributeCode" :readonly="isViewMode" />
				</ElFormItem>
				<ElFormItem prop="attributeName" label="产品属性名称：">
					<ElInput v-model="form.attributeName" :readonly="isViewMode" />
				</ElFormItem>
			</ElForm>
