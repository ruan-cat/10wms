...
function handleDelete(row: TableData) {
	ElMessageBox.confirm("确定要删除这条记录吗？", "提示", {
		...
		.then(async () => {
			try {
				await http.request({ url: "/api/purchase/batch-receiving/delete", method: "post", data: { ids: [row.id] } });
...
async function submitForm() {
	...
	try {
		if (dialogType.value === 0) {
			await http.request({ url: "/api/purchase/batch-receiving/add", method: "post", data: form.value });
			ElMessage.success("新增成功");
		} else if (dialogType.value === 1) {
			await http.request({ url: "/api/purchase/batch-receiving/update", method: "post", data: form.value });
