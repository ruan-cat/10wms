...
async function getList() {
	...
	try {
		...
		const response = await http.requestCompat({
			url: "/api/personnel/work-status/list",
			method: "post",
			data: params,
		});
...
async function handleSubmit() {
	...
	await formRef.value.validate(async (valid) => {
		if (valid) {
			try {
				const url = formData.id ? "/api/personnel/work-status/update" : "/api/personnel/work-status/add";
...
async function handleDelete(row: any) {
	try {
		await http.requestCompat({
			url: "/api/personnel/work-status/delete",
			method: "post",
			data: { id: row.id },
		});
