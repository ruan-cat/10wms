...
const emptyForm: TableData = { dictCode: "", dictName: "", dictType: "", dictValue: "", remark: "" };
		...
		.then(async () => {
			try {
				await http.requestCompat({
					url: "/api/system/dictionary/delete",
					method: "post",
...
function handleDelete(row: TableData) {
	ElMessageBox.confirm("确定要删除这条记录吗？", "提示", {
		...
		.then(async () => {
			try {
				await http.requestCompat({ url: "/api/system/dictionary/delete", method: "post", data: { ids: [row.id] } });
...
async function submitForm() {
	...
	try {
		if (dialogType.value === 0) {
			await http.requestCompat({ url: "/api/system/dictionary/add", method: "post", data: form.value });
			ElMessage.success("新增成功");
		} else if (dialogType.value === 1) {
			await http.requestCompat({ url: "/api/system/dictionary/update", method: "post", data: form.value });
...
<template>
	<div class="dictionary-page">
		...
		<el-dialog
			...
			:title="isViewMode ? '查看字典' : dialogType === 0 ? '新增字典' : '编辑字典'"
			width="600px"
			:before-close="handleDialogClose"
		>
			<ElForm :model="form" label-position="right" label-width="150px">
				<h2>表单信息管理</h2>
				<ElFormItem prop="dictCode" label="字典编码："
					><ElInput v-model="form.dictCode" :readonly="isViewMode"
				/></ElFormItem>
				<ElFormItem prop="dictName" label="字典名称："
					><ElInput v-model="form.dictName" :readonly="isViewMode"
				/></ElFormItem>
				<ElFormItem prop="dictType" label="字典类型："
					><ElInput v-model="form.dictType" :readonly="isViewMode"
				/></ElFormItem>
				<ElFormItem prop="dictValue" label="字典值："
					><ElInput v-model="form.dictValue" :readonly="isViewMode"
				/></ElFormItem>
				<ElFormItem prop="remark" label="备注："
					><ElInput v-model="form.remark" :readonly="isViewMode" type="textarea"
				/></ElFormItem>
			</ElForm>
			<template #footer>
