...
/** 表单数据 */
const formData = ref<AddSupplierParams>({
	supplierCode: "",
	supplierName: "",
	supplierType: "",
	contact: "",
...
const loadData = async () => {
	loading.value = true;
	try {
		const result = await getSupplierList(queryParams);
...
const handleExport = async () => {
	try {
		const blob = await exportSupplier(queryParams);
	...
	<div class="supplier-container">
		...
		<el-card shadow="never" class="table-card">
			...
			<el-table :data="tableData" :loading="loading" border stripe>
				...
				<el-table-column prop="supplierType" label="供应商类型" min-width="100">
					<template #default="{ row }">
						<span v-if="row.supplierType === 'manufacturer'">生产厂家</span>
						<span v-else-if="row.supplierType === 'distributor'">经销商</span>
						<span v-else-if="row.supplierType === 'agent'">代理商</span>
						<span v-else-if="row.supplierType === 'other'">其他</span>
						<span v-else>-</span>
					</template>
				</el-table-column>
