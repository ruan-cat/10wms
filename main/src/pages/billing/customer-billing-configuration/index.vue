...
async function loadData() {
	...
	try {
		const response = await http.requestCompat<any>({
			url: "/api/customer-billing-configuration/list",
			method: "post",
			data: {
		...
		.then(async () => {
			try {
				await http.requestCompat({
					url: "/api/customer-billing-configuration/delete",
					method: "post",
...
async function submitForm() {
	...
	try {
		if (dialogType.value === 0) {
			await http.requestCompat({
				url: "/api/customer-billing-configuration/add",
				method: "post",
				data: form.value,
			});
			...
			await http.requestCompat({
				url: "/api/customer-billing-configuration/update",
				method: "post",
				data: form.value,
			});
...
<template>
	<div class="customer-billing-configuration-page">
		...
		<el-dialog
			...
			:title="isViewMode ? '查看客户计费配置' : dialogType === 0 ? '新增客户计费配置' : '编辑客户计费配置'"
			width="600px"
			:before-close="handleDialogClose"
		>
			<ElForm :model="form" label-position="right" label-width="150px">
				<h2>表单信息管理</h2>
				<ElFormItem prop="code" label="代码：">
...
.customer-billing-configuration-page {
	padding: 20px;
}
