...
const tableColumns = ref<SimpleDataTableColumn<TableData>[]>([
	{ prop: "code", label: "计费商品类代码", width: 200 },
	{ prop: "name", label: "计费商品类名称", width: 200 },
]);
...
async function loadData() {
	...
	try {
		const response = await http.request<any>({
			url: "/api/billing-commodity-category/list",
			method: "post",
			data: {
...
function handleBatchDelete() {
	...
	ElMessageBox.confirm(`确定要删除选中的 ${selectedRows.value.length} 条记录吗？`, "提示", {
		...
		.then(async () => {
			try {
				await http.request({
					url: "/api/billing-commodity-category/delete",
					method: "post",
...
async function submitForm() {
	...
	try {
		if (dialogType.value === 0) {
			// 新增
			await http.request({
				url: "/api/billing-commodity-category/add",
				method: "post",
				data: form.value,
			});
			...
			// 编辑
			await http.request({
				url: "/api/billing-commodity-category/update",
				method: "post",
				data: form.value,
			});
...
<template>
	<div class="billing-commodity-category-page">
		<!-- 操作按钮 -->
		...
		<el-dialog
			...
			:title="isViewMode ? '查看计费商品类' : dialogType === 0 ? '新增计费商品类' : '编辑计费商品类'"
			width="600px"
			:before-close="handleDialogClose"
		>
			<ElForm :model="form" label-position="right" label-width="150px">
				<h2>表单信息管理</h2>
				<ElFormItem prop="code" label="计费商品类代码：">
					<ElInput v-model="form.code" :readonly="isViewMode" />
				</ElFormItem>
				<ElFormItem prop="name" label="计费商品类名称：">
					<ElInput v-model="form.name" :readonly="isViewMode" />
				</ElFormItem>
			</ElForm>
...
.billing-commodity-category-page {
	padding: 20px;
}
