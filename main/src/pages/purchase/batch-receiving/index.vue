...
import { ElButton, ElMessage, ElMessageBox, ElForm, ElFormItem, ElInput, ElInputNumber } from "element-plus";
...
async function loadData() {
	...
	try {
		const response = await http.requestCompat<any>({
			url: "/api/purchase/batch-receiving/list",
			method: "post",
...
const emptyForm: TableData = { receiptNo: "", supplierName: "", goodsName: "", quantity: 0, receiveDate: "" };
		...
		.then(async () => {
			try {
				await http.requestCompat({
					url: "/api/purchase/batch-receiving/delete",
					method: "post",
...
function handleDelete(row: TableData) {
	ElMessageBox.confirm("确定要删除这条记录吗？", "提示", {
		...
		.then(async () => {
			try {
				await http.requestCompat({ url: "/api/purchase/batch-receiving/delete", method: "post", data: { ids: [row.id] } });
...
async function submitForm() {
	...
	try {
		if (dialogType.value === 0) {
			await http.requestCompat({ url: "/api/purchase/batch-receiving/add", method: "post", data: form.value });
			ElMessage.success("新增成功");
		} else if (dialogType.value === 1) {
			await http.requestCompat({ url: "/api/purchase/batch-receiving/update", method: "post", data: form.value });
...
<template>
	<div class="batch-receiving-page">
		...
		<el-dialog
			...
			:title="isViewMode ? '查看批量收货' : dialogType === 0 ? '新增批量收货' : '编辑批量收货'"
			width="600px"
			:before-close="handleDialogClose"
		>
			<ElForm :model="form" label-position="right" label-width="150px">
				<h2>表单信息管理</h2>
				<ElFormItem prop="receiptNo" label="收货单号："
					><ElInput v-model="form.receiptNo" :readonly="isViewMode"
				/></ElFormItem>
				<ElFormItem prop="supplierName" label="供应商："
					><ElInput v-model="form.supplierName" :readonly="isViewMode"
				/></ElFormItem>
				<ElFormItem prop="goodsName" label="商品名称："
					><ElInput v-model="form.goodsName" :readonly="isViewMode"
				/></ElFormItem>
				<ElFormItem prop="quantity" label="数量："
					><ElInputNumber v-model="form.quantity" :readonly="isViewMode" style="width: 100%"
				/></ElFormItem>
				<ElFormItem prop="receiveDate" label="收货日期："
					><ElInput v-model="form.receiveDate" :readonly="isViewMode"
				/></ElFormItem>
			</ElForm>
			<template #footer>
