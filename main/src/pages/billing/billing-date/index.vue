...
import { ElButton, ElMessage, ElMessageBox, ElForm, ElFormItem, ElInput, ElDatePicker } from "element-plus";
...
const tableColumns = ref<SimpleDataTableColumn<TableData>[]>([
	{ prop: "date", label: "计费日期", width: 200 },
	{ prop: "count", label: "是否已计费", width: 150 },
]);
...
<template>
	<div class="billing-date-page">
		...
		<el-dialog
			...
			:title="isViewMode ? '查看计费日期' : dialogType === 0 ? '新增计费日期' : '编辑计费日期'"
			width="600px"
			:before-close="handleDialogClose"
		>
			<ElForm :model="form" label-position="right" label-width="150px">
				<h2>表单信息管理</h2>
				<ElFormItem prop="date" label="计费日期：">
					<ElDatePicker v-model="form.date" type="date" :disabled="isViewMode" style="width: 100%" />
				</ElFormItem>
				<ElFormItem prop="count" label="是否已计费：">
					<ElInput v-model="form.count" :readonly="isViewMode" />
				</ElFormItem>
			</ElForm>
