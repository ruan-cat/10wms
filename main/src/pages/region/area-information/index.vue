...
/** 表格数据 */
const tableData = ref<AreaInfoRow[]>([
	{ id: 1, area_code: "213", area_name: "华东大区" },
	{ id: 2, area_code: "214", area_name: "华南大区" },
]);

/** 表格列配置 */
const tableColumns = ref<SimpleDataTableColumn<AreaInfoRow>[]>([
	{ prop: "area_code", label: "大区代码", width: 200 },
	{ prop: "area_name", label: "大区名称", width: 200 },
...
/** 当前编辑行 */
const currentRow = ref<AreaInfoRow>({
	id: 0,
	area_code: "",
	area_name: "",
});
...
const rules: FormRules = {
	area_code: [{ required: true, message: "请输入大区代码", trigger: "blur" }],
	area_name: [{ required: true, message: "请输入大区名称", trigger: "blur" }],
};
...
/** 处理编辑 */
function handleEdit(row: AreaInfoRow) {
...
/** 处理删除 */
function handleDelete(row: AreaInfoRow) {
	...
	<div class="area-info-container">
		<el-card shadow="never">
			<template #header>
				<div class="card-header">
					<span class="font-bold">大区信息</span>
				</div>
			</template>
		...
		<el-dialog v-model="dialogVisible" :title="dialogTitle" width="500px" draggable destroy-on-close>
			<el-form ref="formRef" :model="currentRow" :rules="rules" label-width="120px">
				<el-form-item label="大区代码" prop="area_code">
					<el-input v-model="currentRow.area_code" />
				</el-form-item>
				<el-form-item label="大区名称" prop="area_name">
					<el-input v-model="currentRow.area_name" />
				</el-form-item>
			</el-form>
			<template #footer>
