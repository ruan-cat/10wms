		...
		.then(async () => {
			try {
				await http.requestCompat({
					url: "/api/warehousing-rate/delete",
					method: "post",
...
function handleDelete(row: TableData) {
	ElMessageBox.confirm("确定要删除这条记录吗？", "提示", {
		...
		.then(async () => {
			try {
				await http.requestCompat({ url: "/api/warehousing-rate/delete", method: "post", data: { ids: [row.id] } });
...
async function submitForm() {
	...
	try {
		if (dialogType.value === 0) {
			await http.requestCompat({ url: "/api/warehousing-rate/add", method: "post", data: form.value });
			ElMessage.success("新增成功");
		} else if (dialogType.value === 1) {
			await http.requestCompat({ url: "/api/warehousing-rate/update", method: "post", data: form.value });
...
<template>
	<div class="warehousing-rate-page">
		...
		<el-dialog
			...
			:title="isViewMode ? '查看入库费率' : dialogType === 0 ? '新增入库费率' : '编辑入库费率'"
			width="600px"
