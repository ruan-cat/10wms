...
const tableColumns = ref<SimpleDataTableColumn<TableData>[]>([
	{ prop: "statusCode", label: "品检状态代码", width: 200 },
	{ prop: "statusName", label: "品检状态名称", width: 200 },
]);
...
async function loadData() {
	...
	try {
		const response = await http.requestCompat<any>({
			url: "/api/quality-inspection-status/list",
			method: "post",
			data: {
		...
		.then(async () => {
			try {
				await http.requestCompat({
					url: "/api/quality-inspection-status/delete",
					method: "post",
...
async function submitForm() {
	...
	try {
		if (dialogType.value === 0) {
			// 新增
			await http.requestCompat({
				url: "/api/quality-inspection-status/add",
				method: "post",
				data: form.value,
			});
			...
			// 编辑
			await http.requestCompat({
				url: "/api/quality-inspection-status/update",
				method: "post",
				data: form.value,
			});
...
<template>
	<div class="quality-inspection-status-page">
		<!-- 操作按钮 -->
		...
		<el-dialog
			...
			:title="isViewMode ? '查看品检状态' : dialogType === 0 ? '新增品检状态' : '编辑品检状态'"
			width="600px"
			:before-close="handleDialogClose"
		>
			<ElForm :model="form" label-position="right" label-width="150px">
				<h2>表单信息管理</h2>
				<ElFormItem prop="statusCode" label="品检状态代码：">
					<ElInput v-model="form.statusCode" :readonly="isViewMode" />
				</ElFormItem>
				<ElFormItem prop="statusName" label="品检状态名称：">
					<ElInput v-model="form.statusName" :readonly="isViewMode" />
				</ElFormItem>
			</ElForm>
...
.quality-inspection-status-page {
	padding: 20px;
}
